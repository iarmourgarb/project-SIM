...
// renamed event to newSong
// renamed activeItem to adding
...
//name is song or artist, value is if we're adding the song or the artist
    let { name, value } = newSong.target;

    //something else has to change here
    if (newSong.target.type === "checkbox") {
    ...
    const newSong = { ...this.state.adding, [name]: value };
  ...
  render() {
    ...
    return (
      ...
      <Modal isOpen={true} toggle={toggle}>
        <ModalHeader toggle={toggle}> Adding Songs </ModalHeader>
        <ModalBody>
          <Form>
            ...
            <FormGroup>
              ...
              <Input
                type="text"
                name="artist"
                value={this.state.adding.artist}
                onChange={this.handleChange}
                placeholder="Enter artist name"
              />
            </FormGroup>

// here we need a button to add to database??
            <FormGroup check>
              <Label for="completed">
                <Input
                  type="checkbox"
                  name="completed"
                  checked={this.state.adding.completed}
                  onChange={this.handleChange}
                />
                Completed
              </Label>
        ...
        <ModalFooter>
          <Button color="success" onClick={() => onSave(this.state.adding)}>
            Save
          </Button>
        </ModalFooter>
      </Modal>
